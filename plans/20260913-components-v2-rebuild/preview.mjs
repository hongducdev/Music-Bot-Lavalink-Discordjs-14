...
const cards = [
  ...
  ['Tạm dừng · lặp một bài', buildNowPlayingCard({ ...player, paused: true, repeatMode: 'track' })],
  ['Trực tiếp · không có ảnh', buildNowPlayingCard({ ...player, queue: { current: { ...song, info: { ...song.info, title: 'Radio 24/7', isStream: true, artworkUrl: null } }, tracks: [] } })],
...
function markdown(s){if(s.includes(String.fromCharCode(96).repeat(3)))return s.split(String.fromCharCode(96).repeat(3)).map((part,i)=>i%2?'<pre style="margin:0;max-width:100%;box-sizing:border-box">'+escape(part.trim())+'</pre>':markdown(part)).join('');return escape(s).replace(/^### (.+)$/gm,'<h3>$1</h3>').replace(/^-# (.+)$/gm,'<span class="small">$1</span>').replace(/\*\*(.+?)\*\*/g,'<strong>$1</strong>').replace(/\x60([^\x60]+)\x60/g,'<code>$1</code>').replace(/\[([^\]]+)\]\([^\n]+?\)/g,'<a>$1</a>');}
